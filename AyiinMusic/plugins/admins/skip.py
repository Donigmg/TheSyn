...
from AyiinMusic.utils.database import get_loop
from AyiinMusic.utils.decorators import AdminRightsCheck
from AyiinMusic.utils.inline.play import (stream_markup,
                                          telegram_markup)
from AyiinMusic.utils.stream.autoclear import auto_clean
...
async def skip(cli, message: Message, _, chat_id):
    if not len(message.command) < 2:
                        ...
                        for x in range(state):
                            ...
                            if not check:
                                try:
                                    ...
                                    await Ayiin.stop_stream(chat_id)
                                except:
                                    return
                                break
                    else:
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Ayiin.stop_stream(chat_id)
                except:
                    return
        except:
            try:
    ...
    if "live_" in queued:
        ...
        try:
            await Ayiin.skip_stream(chat_id, link, video=status)
        except Exception:
        ...
        await message.reply_photo(
            ...
            caption=_["stream_1"].format(
                f"[{title[:25]}](https://t.me/{app.username}?start=info_{videoid})", check[0]["dur"], user,
            ),
        ...
        try:
            await Ayiin.skip_stream(chat_id, file_path, video=status)
        except Exception:
            return await mystic.edit_text(_["call_9"])
        ...
        try:
            await Ayiin.skip_stream(chat_id, videoid, video=True)
        except Exception:
            return await message.reply_text(_["call_9"])
        button = telegram_markup(_)
        await message.reply_photo(
        ...
        try:
            await Ayiin.skip_stream(chat_id, queued, video=status)
        except Exception:
            return await message.reply_text(_["call_9"])
        if videoid == "telegram":
            button = telegram_markup(_)
